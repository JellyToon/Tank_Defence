...
bool Billboard::Initialize(ID3D11Device* device, const char* modelFilename, const WCHAR* textureFilename)
{
	// 모델 데이터를 로드합니다.
...
void Billboard::ShutdownBuffers()
{
	// 인덱스 버퍼를 해제합니다.
	if (m_indexBuffer)
	{
...
bool Billboard::LoadTexture(ID3D11Device* device, const WCHAR* filename)
{
	// 텍스처 오브젝트를 생성한다.
...
bool Billboard::LoadModel(const char* filename)
{
	// 모델 파일을 엽니다.
	ifstream fin;
	fin.open(filename);
